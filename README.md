...
│   ├── ingredients.json     # Vietnamese food database (15+ items)
...
│   ├── tracking-hooks.ts    # Tracking utility hooks
│   ├── updateStreak.ts      # Streak calculation helper
...
**Integrated Systems:** Google Analytics + Sentry Error Tracking + Custom Logger
...
2. **Configure Environment Variables:**
   ...
   - Add to `.env.local`:
     ```
     NNEXT_PUBLIC_GA_ID=G-XXXXXXXXXX
NEXT_PUBLIC_SENTRY_DSN=https://xxxxx@oxxxxxx.ingest.sentry.io/xxxxx
NEXT_PUBLIC_APP_VERSION=0.1.0
DATABASE_URL=postgresql://user:password@localhost:5432/calorie_db
DIRECT_URL=postgresql://user:password@localhost:5432/calorie_db
     ```
3. **Tracking Features:**

Google Analytics: Page view tracking (automatic), event tracking, user properties
Sentry: Error tracking, performance monitoring, release tracking
Custom Logger: debug, info, warn, error levels

4. **See [TRACKING_SETUP.md](./TRACKING_SETUP.md) for detailed instructions**

---

## 📄 Pages Hiện Có

| URL | Tên Component | Mô Tả | Status |
|-----|---------------|-------|--------|
| `/` | `app/page.tsx` | Dashboard/Home - hiển thị ProfileForm nếu chưa setup, hoặc DashboardPage | ✅ Active |
| `/diary` | `app/diary/page.tsx` | Giao diện nhập nhật ký calo hàng ngày | ✅ Active |
| `/settings` | `app/settings/page.tsx` | Cài đặt hồ sơ người dùng, thiết lập TDEE & macros | ✅ Active |
| `/stats` | `app/stats/page.tsx` | Thống kê tuần: streak, calories, macros, biểu đồ | ✅ Active |
...
|--------|----------|-------|-----------|
| **GET** | `/api/ingredients` | Lấy toàn bộ danh sách nguyên liệu | - |
| **GET** | `/api/users` | Tìm user theo email | `?email=...` |
| **POST** | `/api/users` | Tạo mới hoặc upsert user | `{ email, name? }` |
| **GET** | `/api/meals` | Lấy meals của user theo ngày | `?userId=...&date=...` |
| **POST** | `/api/meals` | Tạo meal mới | `{ userId, mealType, date, ingredients[] }` |
| **GET** | `/api/meals/[id]` | Lấy chi tiết meal | `[id]` (path) |
...
**Database Integration** | Prisma ORM + PostgreSQL schema (User, Meal, Ingredient, MealIngredient) |
| **API Routes** | 6 API endpoints cho ingredients, users, meals |
| **Analytics** | Google Analytics page tracking + event tracking |
| **Error Tracking** | Sentry error tracking + performance monitoring |
| **Custom Logger** | Structured logging system (debug, info, warn, error) |
...
Meal history & detailed analytics
...
**Version:** 0.1.0  
**Status:** v0.1.0 - Early Stage Development
